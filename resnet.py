...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=7,if_include_top=True):
        self.inplanes = 64
...
class ResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=7, num_group=32,if_include_top=True):
        self.inplanes = 64
        ...
        if if_include_top:
            self.fc = nn.Linear(512 * block.expansion, num_classes)
        self.in_clude_top = if_include_top
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
...
def resnext50(pretrained = False,**kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load('/home/server/Documents/my_net/resnext50_32x4d.pth'),strict=False)
    return model
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load('/home/server/Documents/my_net/resnet50.pth'),strict=False)
    return model
...
def resnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load('/home/server/Documents/my_net/resnet101.pth'),strict=False)
    return model
